...
fn test_global_defaults_load_without_persona() {
    ...
    let has_snooze = config
        .rules
        .iter()
        .any(|r| r.name == "Global Snooze Baseline");
...
fn test_web_dev_persona_adds_node_modules_rule() {
    ...
    assert!(
        node_modules_rule.is_some(),
        "web-dev persona must add node_modules rule"
    );
...
fn test_persona_override_tracks_source_field() {
    ...
    let node_modules_rule = config
        .rules
        .iter()
        .find(|r| r.name == "Nuke node_modules")
        .unwrap();
...
fn test_unknown_persona_errors() {
    ...
    assert!(
        result.is_err(),
        "Loading an unknown persona should return an error"
    );
...
fn test_noise_patterns_loaded_from_defaults() {
    ...
    assert!(
        ...
        "Noise patterns should be loaded"
    );

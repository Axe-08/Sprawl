...
fn test_sentinel_end_to_end_classification() {
    ...
    assert!(matches!(result.status, SecretClassification::KnownProvider(_)));
    ...
    assert!(matches!(result2.status, SecretClassification::FilteredNoise(_)));
    
    // 3. Ambiguous High Entropy
    let config_content = fs::read_to_string(project_path.join("config.yaml")).unwrap();
    let config_val = config_content.split(": ").nth(1).unwrap().trim().trim_matches('"');

...
fn provider_prefixes() -> &'static [(Regex, &'static str)] {
    ...
    PREFIXES.get_or_init(|| {
        vec![
            ...
            (Regex::new(r"^glpat-[A-Za-z0-9\-]{20,}").unwrap(), "GitLab PAT"),
            (Regex::new(r"^xox[bpors]-[0-9a-zA-Z\-]+").unwrap(), "Slack Token"),
            (Regex::new(r"^AIza[0-9A-Za-z\-_]{35}").unwrap(), "Google API Key"),
...
mod tests {
    ...
    fn test_known_provider_classification() {
        ...
        assert_eq!(res.status, SecretClassification::KnownProvider("Stripe Live".to_string()));

        let res2 = classify_string("AKIAIOSFODNN7EXAMPLE");
        assert_eq!(res2.status, SecretClassification::KnownProvider("AWS Access Key".to_string()));
    }

    #[test]
    fn test_negative_filter_classification() {
        let res = classify_string("123e4567-e89b-12d3-a456-426614174000");
        assert_eq!(res.status, SecretClassification::FilteredNoise("UUID".to_string()));

        let res2 = classify_string("f1d2d2f924e986ac86fdf7b36c94bcdf32beec15");
        assert_eq!(res2.status, SecretClassification::FilteredNoise("Git SHA".to_string()));
    }

...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        assert!(ciphertext.len() > plaintext.len(), "Ciphertext should include nonce and MAC");
        
        let decrypted = encryptor.decrypt(&ciphertext).unwrap();
        assert_eq!(plaintext, decrypted, "Decrypted text should match original plaintext");
    }
    ...
    fn test_decryption_fails_on_tamper() {
        ...
        assert!(encryptor.decrypt(&tampered).is_err(), "Decryption must fail if ciphertext or MAC is tampered");
    }
}
